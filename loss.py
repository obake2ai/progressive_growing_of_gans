...
def G_wgan_acgan_can(G, D, opt, training_set, minibatch_size,
    ...
    can_level   = 1): # Parameter for creativety
...
def D_wgangp_acgan(G, D, opt, training_set, minibatch_size, reals, labels,
    ...
    cond_weight     = 0.3):     # Weight of the conditioning terms.
...
def D_wgangp_acgan_can(G, D, opt, training_set, minibatch_size, reals, labels,
    ...
    can_level       = 0.3,      # Parameter for creativety
    ...
    with tf.name_scope('GradientPenalty'):
        mixing_factors = tf.random_uniform([minibatch_size, 1, 1, 1], 0.0, 1.0, dtype=fake_images_out.dtype)
        mixed_images_out = tfutil.lerp(tf.cast(reals, fake_images_out.dtype), fake_images_out, mixing_factors)
        mixed_scores_out, mixed_labels_out, mixed_class_out, mixed_class_logits = fp32(D.get_output_for(mixed_images_out, is_training=True))
        mixed_scores_out = tfutil.autosummary('Loss/mixed_scores', mixed_scores_out)
        mixed_loss = opt.apply_loss_scaling(tf.reduce_sum(mixed_scores_out))
        mixed_grads = opt.undo_loss_scaling(fp32(tf.gradients(mixed_loss, [mixed_images_out])[0]))
        mixed_norms = tf.sqrt(tf.reduce_sum(tf.square(mixed_grads), axis=[1,2,3]))
